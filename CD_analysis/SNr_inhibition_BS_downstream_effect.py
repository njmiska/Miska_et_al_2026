...
if only_plot_FR == 0:
    import pickle
    clusters_info_DF.to_pickle('~/python/saved_figures/' + condition + '_' + onset_alignment + '_SNrDownstream_' + figure_prefix + '.pkl')
    ...
    with open(condition + '_' + onset_alignment + '_SNrDownstream_' + figure_prefix + '.pickle', 'wb') as f:
...
################# LOAD
import pickle
clusters_info_DF = pd.read_pickle('~/python/saved_figures/' + condition + '_' + onset_alignment + '_SNrDownstream_' + figure_prefix + '.pkl')

with open(condition + '_' + onset_alignment + '_SNrDownstream_' + figure_prefix + '.pickle', 'rb') as f:
    data = pickle.load(f)
...
# indices_to_use = logical_and_multiple([clusters_info_DF['Allenregion'].isin(allen_regions_formidbrain), clusters_info_DF['BS_score'] == 1, clusters_info_DF['pid'].isin(pids_list_SNr_contra_trained), clusters_info_DF['IBL_label'] > 0.6])
...
# indices_to_use = logical_and_multiple([clusters_info_DF['Allenregion'].isin(allen_regions_formidbrain), clusters_info_DF['BS_score'] == 1, clusters_info_DF['IBL_label'] == 1.0])
...
# ###### ALL IPSI UNITS
# indices_to_use = logical_and_multiple([clusters_info_DF['Allenregion'].isin(allen_regions_formidbrain), clusters_info_DF['BS_score'] == 1, clusters_info_DF['IBL_label'] == 0.6, clusters_info_DF['pid'].isin(pids_list_SNr_trained)])
# title_str = 'All BS ipsi units'

# ###### ALL CONTRA UNITS
# indices_to_use = clusters_info_DF['pid'].isin(pids_list_ZI_trained_contra) | clusters_info_DF['pid'].isin(pids_list_SNr_contra_trained) & clusters_info_DF['BS_score'] == 1#
...
#################################################################################################
indices_to_use_plt = indices_to_use.astype(bool)
...
smoothed_traces_list_nonstim = []
smoothed_traces_list_stim = []
bin_size = 0.05
window_ms = 300
...
for i in range(0,len(selected_arrays_stim)):
    ...
    ### smoothing
    smoothed_trace_nonstim = rolling_window_mean_1d(abs_selected_arrays_nonstim[i], window_bins)
    smoothed_trace_stim = rolling_window_mean_1d(abs_selected_arrays_stim[i], window_bins)
    smoothed_traces_list_nonstim.append(smoothed_trace_nonstim)
    smoothed_traces_list_stim.append(smoothed_trace_stim)

mean_delta_trace_nonstim = np.nanmean(smoothed_traces_list_nonstim, axis=0) * 100
mean_delta_trace_stim = np.nanmean(smoothed_traces_list_stim, axis=0) * 100
stderr_mean_delta_trace_nonstim = np.std(smoothed_traces_list_nonstim,axis=0)/np.sqrt(len(smoothed_traces_list_nonstim)) * 100
stderr_mean_delta_trace_stim = np.std(smoothed_traces_list_stim,axis=0)/np.sqrt(len(smoothed_traces_list_stim)) * 100
